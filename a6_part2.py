...
def compare_predictions(y_test, predictions, num_examples=5):
    ...
    for i in range(min(num_examples, len(y_test))):
        ...
        print(
            f"${actual:>13,.2f}   ${predicted:>13,.2f}   ${error:>10,.2f}   {pct_error:>6.2f}%"
        )
...
def make_prediction(model, sqft, bedrooms, bathrooms, age):
    ...
    print(
        f"House specs: {sqft} sq ft, {bedrooms} bedrooms, {bathrooms} bathrooms, {age} years old"
    )
...
if __name__ == "__main__":
    ...
    make_prediction(model, 2000, 3, 2, 10)
